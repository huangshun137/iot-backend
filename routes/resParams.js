...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (_id) {
    ...
    const existingParams = await ResParams.findById(_id);
    if (!existingParams) {
    ...
    const updatedParams = await ResParams.findByIdAndUpdate(
      _id,
    ...
    const newParams = await ResParams.create({
      ...data,
      command: commandId
    });
...
router.get('/', asyncHandler(async (req, res) => {
  try {
    ...
    const params = await ResParams.find(query);
    res.json(params);
  } catch (err) {
...
router.delete('/:id', asyncHandler(async (req, res) => {
  try {
    ...
    const deletedParams = await ResParams.findByIdAndDelete(paramsId);
    if (!deletedParams) {
...
async function createBatch(paramsList) {
  ...
  const newParams = await ResParams.insertMany(paramsList.map(param => ({
    ...param,
...
async function updateBatch(updatesList) {
  ...
  const updatePromises = updatesList.map(async update => {
    ...
    return ResParams.findByIdAndUpdate(
      _id,
      { ...data, command: commandId || existingParams.command },
      { new: true, runValidators: true }
    );
...
async function deleteBatch(ids) {
  ...
  await ResParams.deleteMany({ _id: { $in: ids } });
}
